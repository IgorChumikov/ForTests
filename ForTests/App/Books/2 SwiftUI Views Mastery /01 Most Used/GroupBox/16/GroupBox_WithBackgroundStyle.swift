...
/*
 GroupBox — использование backgroundStyle для фона (iOS 16+)

 • Начиная с iOS 16, GroupBox поддерживает модификатор .backgroundStyle(),
   позволяющий задавать фон напрямую через стили.
 • В качестве стиля можно использовать:
      – материалы (например, .thinMaterial для эффекта blur);
      – цвета и системные цвета (.secondary, .tertiary и др.);
      – градиенты и любые ShapeStyle.
 • Отлично подходит для создания карточек профиля, панелей настроек,
   информационных блоков и оформленных секций.
 • backgroundStyle работает поверх всего содержимого GroupBox, включая рамку.
 */
