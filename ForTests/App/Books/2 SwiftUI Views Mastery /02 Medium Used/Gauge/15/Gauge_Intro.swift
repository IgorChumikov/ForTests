...
/*
 Gauge — индикатор уровня или прогресса (iOS 15+)

 • Показывает значение в диапазоне (по умолчанию от 0 до 1).
 • Используется для отображения заряда, громкости, загрузки, температуры и других величин.
 • Поддерживает разные системные стили: .linearCapacity, .accessoryLinear, .accessoryLinearCapacity и др.
 • Стиль и цвет можно менять через gaugeStyle и tint.
 • Лёгок в использовании — достаточно передать value и при желании label.
 */
