...
/*
 LabeledContent — связанная пара «лейбл + значение» (iOS 16+)

 • Универсальный контейнер для отображения пары: заголовок (Label) и значение (Value).
 • Автоматически подстраивается под разные контейнеры (VStack, Form, List),
   сохраняя системный вид и правильное выравнивание.
 • Позволяет использовать:
      – простой текст (value:)
      – кастомные вью (closure)
      – автоформатирование чисел, процентов и дат.
 • Отлично подходит для форм, настроек, профилей, статистики и списков характеристик.
 • Является более гибкой альтернативой badge, т.к. даёт полный контроль над оформлением значения.
 */
