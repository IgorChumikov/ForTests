...
/*
 LazyHGrid — управление расстояниями между элементами (iOS 14+)

 • Появился в SwiftUI начиная с iOS 14.
 • Позволяет создавать горизонтальную ленточную сетку с гибкой настройкой.
 • columnSpacing — расстояние между колонками (по горизонтали).
 • rowSpacing — расстояние между строками (по вертикали).
 • Слайдеры позволяют динамически изменять отступы и сразу видеть результат.
 • Удобно для визуальной настройки сеток, пикеров и галерей.
 */
