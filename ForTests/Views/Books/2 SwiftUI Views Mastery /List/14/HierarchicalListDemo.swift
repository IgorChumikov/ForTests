...
/*
 List — глубокие иерархические структуры с children (iOS 14+)

 • Позволяет отображать вложенные данные любой глубины: SwiftUI рекурсивно
   проходит через children и автоматически создаёт раскрывающиеся уровни.
 • List(items, children: \.children) работает с любыми моделями Identifiable,
   достаточно указать key path к массиву дочерних элементов.
 • SwiftUI сам показывает Disclosure-индикаторы, управляет раскрытием и анимациями.
 • Идеально подходит для древовидных структур: папки, каталоги, подразделения,
   семейные деревья, вложенные категории и любые рекурсивные данные.
 */
